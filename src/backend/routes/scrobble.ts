...
router.post('/track', async (req: Request, res: Response) => {
  try {
    ...
    const success = await lastfmService.scrobbleTrack(track);
...
router.post('/batch', async (req: Request, res: Response) => {
  try {
    ...
    const tracksWithTimestamps = tracks.map((track, index) => ({
      ...track,
      timestamp: track.timestamp || (currentTime - (tracks.length - index - 1) * 180) // 3 minutes apart
    }));
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Batch scrobbling failed'
    });
  }
});
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to get scrobble history'
    });
  }
});
...
router.get('/session/:sessionId', async (req: Request, res: Response) => {
  try {
    ...
    const session = await fileStorage.readJSON(`scrobbles/session-${sessionId}.json`);
...
router.post('/prepare-from-release', async (req: Request, res: Response) => {
  try {
    ...
    const tracksToScrobble = selectedTracks && selectedTracks.length > 0
      ? release.tracklist.filter((track: any, index: number) => selectedTracks.includes(index))
      : release.tracklist;
    
    // Parse start time (expecting Unix timestamp in seconds)
    const startTimestamp = startTime ? parseInt(startTime) : Math.floor(Date.now() / 1000);
    
    // Calculate timestamps based on track durations
    let currentTime = startTimestamp;
    const scrobbleTracks: ScrobbleTrack[] = tracksToScrobble.map((track: any) => {
      const trackDuration = track.duration ? parseTrackDuration(track.duration) : 180; // Default 3 minutes
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to prepare tracks'
    });
  }
});
...
router.get('/progress/:sessionId', async (req: Request, res: Response) => {
  try {
    ...
    const session = await fileStorage.readJSON<ScrobbleSession>(`scrobbles/session-${sessionId}.json`);
