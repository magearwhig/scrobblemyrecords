...
const TestComponent: React.FC = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={() =>
          dispatch({ type: 'SET_SERVER_URL', payload: 'http://localhost:4000' })
        }
      >
        Set Server URL
      </button>
...
describe('AppContext', () => {
  it('provides initial state correctly', () => {
    ...
    expect(screen.getByTestId('server-url')).toHaveTextContent(
      'http://localhost:3001'
    );
  });
  ...
  it('maintains state immutability', () => {
    ...
    if (capturedStates.length >= 2) {
      expect(capturedStates[0]).not.toBe(
        capturedStates[capturedStates.length - 1]
      );
    }
  });
});
