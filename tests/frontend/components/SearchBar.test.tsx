...
describe('SearchBar', () => {
  ...
  it('shows overlay when disabled', () => {
    ...
    const overlay = document.querySelector('div[style*="rgba(255, 255, 255, 0.7)"]');
  ...
  it('clears input and calls onSearch when clear button is clicked', async () => {
    const onSearch = jest.fn();
    render(<SearchBar {...defaultProps} defaultValue="test" onSearch={onSearch} />);
  ...
  it('calls onSearch with default value on mount', () => {
    const onSearch = jest.fn();
    render(<SearchBar {...defaultProps} defaultValue="initial" onSearch={onSearch} />);
  ...
  it('cleans up timeout on unmount', () => {
    const onSearch = jest.fn();
    const { unmount } = render(<SearchBar {...defaultProps} onSearch={onSearch} />);
